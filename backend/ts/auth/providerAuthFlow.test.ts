...
async function fixture(currentAccount?: ProviderAccount) {
    ...
    const dependencies: ProviderAuthFlowDependencies = {
        ...
        accounts: {
            ...
            async link(target, suppliedPassword, identity) {
                step('link');
                linkedTargets.push([target, suppliedPassword, identity]);
...
test('plain request data cannot satisfy the trusted context type', () => {
    const plain = { bindingHash: Buffer.alloc(32), account: null };
...
test('explicit linking passes only the server-resolved UUID target, unchanged password and verified identity', async () => {
    ...
    assert.deepEqual(f.linkedTargets, [[{ userId: account.userId, accountId: account.accountId }, password,
        { provider: 'google', subject: 'opaque-provider-subject' }]]);
