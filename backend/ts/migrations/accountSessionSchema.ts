...
export async function verifyAccountSessionSchema(connection: MigrationConnection): Promise<void> {
    ...
    `), [column('session_hash', 'binary(32)'), column('account_uuid', 'char(36)', 'ascii', 'ascii_bin'),
    ...
    `)).map(row => ({ ...row, sequence: Number(row.sequence), nonUnique: Number(row.nonUnique) })), [
        index('idx_account_sessions_account_created', 'account_uuid', 1),
...
export async function verifyOptionalAccountSessionSchema(connection: MigrationConnection): Promise<void> {
    ...
    const recorded = await rows(connection,
        'SELECT version FROM schema_migrations WHERE version = ?', [ACCOUNT_SESSION_MIGRATION_VERSION]);
